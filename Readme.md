...
## Conclusion
The Wheel odometry show the massive drift due to the wheel slip and no ground truth sensor. The EKF using the IMU fusion with the kinematics model. Make the accuracy difference to the wheel odometry. Because of the IMU integrated, the change of angle can be used to correct the orientation of the robot. And with the Kalman's model the noise from sensor and model can be integrated together make the data more reliable but still drifting due to the measurement error and no ground truth sensor. The Best methodology is to use the EKF with the ICP. With ICP the Lidar can data can be used to correct the translation and rotation of the robot. The EKF only odometry rely on 3 measurement ($v, \theta, \omega$) but with ICP the EKF can rely on 5 measurement ($x, y, v, \theta, \omega$) make ICP more reliable than the EKF only odometry. The problems of the ICP is the empty hallway aliasing. With the same wall, The ICP can recognize the new wall as a same wall and predict that the robot have not moved. This make the wall with obstacle make ICP do localization and mapping more correctly and make ICP with EKF odometry is the most trustworthy data then wheel odometry and EKF only odometry.
